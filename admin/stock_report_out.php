                ...
                <table class="table table-bordered">
                  ...
                  <?php $i=0;  $sum=0;  $sum1=0;  $sum2=0; 
                    ...
                    while ( $d=$data->fetch(PDO::FETCH_ASSOC)) { ++$i;
                      $sale=$obj->db->query("select sum(quantity) as s from stock_out where date between '".$date1."' and '".$date2."'and product_id=".$d['id'])->fetch(PDO::FETCH_ASSOC);
                      $parchase=$obj->db->query("select sum(quantity) as p from stock_in where date between '".$date1."' and '".$date2."'and product_id=".$d['id'])->fetch(PDO::FETCH_ASSOC);
                      ...
                      $return=$obj->db->query("select sum(quantity) as r from returns where date between '".$date1."' and '".$date2."'and product_id=".$d['id'])->fetch(PDO::FETCH_ASSOC);

                      $date=$obj->db->query("select date  from stock_in where date ='".$date1."'and product_id=".$d['id'])->fetch(PDO::FETCH_ASSOC)['date'];
                      ...
                      <tr>   
                        ...
                        <td class="text-right"><?php echo number_format($a=($stv*$dv)/100,2); $sum1+=$a; ?></td>
                        <td class="text-right"><?php echo number_format($cv=($stv-($a*$day)/365),2); $sum2+=$cv; ?></td>
                      </tr>
